...
| CAT-001–006 | Strict curated importer, configurable rarity vocabulary, complete optional metadata, stable UUIDs, conflict rejection, idempotency, history retention in `catalog.test.ts`. The checked-in JSON snapshot has per-item provenance and verification dates. | Verify production catalog paths resolve to published images. |
...
| LEGAL-001–002 | Public Terms/Privacy and unofficial notice; authenticated links; catalog artwork published consistently across environments. | Set the real SUPPORT_CONTACT_URL before launch; operator policy review and approved provenance remain release checks. |
...
The source-artwork publisher runs before development and production builds. Both environments publish the same catalog artwork.
