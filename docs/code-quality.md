...
| Local social graph | `apps/api/src/friends.ts` | Exact-username lookup, canonical relationships, blocks, collection helper access, and comparisons. |
...
- A verified first passkey creates the local user and session with the required username inside the library-managed registration transaction, avoiding incomplete accounts from abandoned ceremonies.
...
- Friend discovery is exact-username only. Removing the Epic friend graph also removes upstream outage, permission, omission, token-refresh, and nickname states from the product.
